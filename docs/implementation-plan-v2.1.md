...
| 1.1 | 2026-08-10 | **T-88 を足した**（#230）。v2.1.0 を配ったあとに見つけた不具合であり、**箱ダイヤに途中入庫が出ていなかった。** 出ないだけでなく、**車庫での待機が「折返し 0 分」として描かれていた**——絵が事実と食い違う種類の不具合であるため、次の版を待たずに直す。 |
...
| T-88 | 箱ダイヤに途中入庫を描く | #230 | S | — |

**規模の目安は v1.0 と同じ**（S = 約 0.5 人日、M = 約 1.5 人日）。合計はおよそ **4.5 人日**。

**T-88 は v2.1.0 を配ったあとに足した。** ほかの 4 件と違い、**動くものを見て出た指摘ではなく、実装が仕様の抜けをそのまま通していた**ものである（[§5.4](#t-88-箱ダイヤに途中入庫を描く)）。
...
**目的**: #230 を直す。仕様書 v2 §5.5.3 に**抜けていた決めを足す。**

**背景**: 仕様書 v2 §5.5.4 の表は `Block.standbys` を**「箱ダイヤでの役割: 営業所待機」として挙げている。** 導出も済んでいる。**描き方を決めていたのは §5.5.3 だけで、そこには運用の端しか書いていなかった。**

> **値は用意されていて、描く決めだけが無い。** 実装は `standbys` を一度も読まないまま通り、レビューでも気づかれなかった——**読まれていない値があることは、絵を見ても分からない。**
...
`layoverMinutes` は**直前の便**（＝入庫回送）からの差であり、入庫の直後に出庫が続くと 0 になる。**紙に刷って配るものが、事実と違うことを言っていた。**

**作業内容**

- [ ] `ChartBlock` に `standbys` を持たせる（`Block.standbys` から段に対応づける）
- [ ] **便 ID の綴りを読まない**——展開した回送（`t1#in`）でも、`route.json` の回送パターンを直接使った便でも同じように効く形にする
- [ ] 車庫から出てきた段の `layoverMinutes` を `null` にする
- [ ] 端と**同じマークを同じ規則で**途中にも置く
- [ ] 車庫を挟む段は**縦線で繋がない**
- [ ] 車庫に居た分を字で添える

**受入条件**

- [ ] 1 日に 2 回出庫する運用で、**途中の入庫と出庫にマークが出る**
- [ ] **「折返し 0 分」が出ない**
- [ ] 車庫に居た分が字で出る
- [ ] **車庫を挟む段が縦線で繋がれない**
- [ ] 途中入庫の無い運用は今までどおり繋がれる
- [ ] 端のマークは今までどおり出る
