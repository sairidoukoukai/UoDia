...
| 作成日 | 2026-08-04 |
| 対象 | UoDia v1.1 |
| 前提仕様書 | [specification-v1.1.md](specification-v1.1.md) v1.4（確定） |
...
**v1.0 の実装計画書を置き換えない。** タスク番号は T-55 の続き（**T-56〜T-65**）とする。方針（[§2 開発方針](implementation-plan.md)）・技術的な設計判断（§3）・issue 化の方針（§7）は v1.0 の本書をそのまま引く。
...
| T-64 | 輸送能力（系統ごとの定員） | #162 | N3 | v1 | M | T-63 |
| T-65 | フォーカス範囲と計算パネル | #166 | N3 | v1 | L | T-63, T-64 |
...
| --- | --- | --- | --- |
| N1 | 3 件 | S 3 | 1.5 人日 |
| N2 | 4 件 | S 2 / M 2 | 4.0 人日 |
| N3 | 3 件 | M 2 / L 1 | 7.0 人日 |
| **合計** | **10 件** | **S 5 / M 4 / L 1** | **約 12.5 人日** |
...
### 4.4 距離と輸送力の導出（T-63・T-64）

**どちらも `domain/` に置く純関数とする。** 便・運用・区間表を受け取り、数を返す。ストアも画面も知らない。

| 関数 | 置き場所 |
| --- | --- |
| 便の距離 | `domain/trip/` |
| 走行距離・営業距離 | `domain/block/` |
| 断面輸送力 | `domain/block/`（フォーカス範囲を引数で受ける） |

**フォーカス範囲は引数であって状態ではない**（T-65）。関数の側に「今どこを見ているか」を持たせない。
...
- [ ] **`.deb` の `Maintainer`、`.app` の `NSHumanReadableCopyright` が同じ表記になっている**

**残っている確認**（#158 のコメントの受入条件）

最後の 2 つは**配布物そのものを見ないと確かめられない**。CI は 3 OS 分を artifact に残しているため（README の「タグを打たずに配布物を取る」）、次のいずれかで確かめる。
...
> **Tauri がどの値を使うかは決まっている。** `.deb` の `Maintainer` と `.app` の `NSHumanReadableCopyright` は、それぞれ `bundle.publisher` と `bundle.copyright` から作られる。**それでも実物を見るのは、`Cargo.toml` の `authors`（`sairidoukoukai`）へ倒れる経路があるため**である。設定を読んだだけでは、どちらが出ているか分からない。
...
- [ ] `Segment.distanceMeters` をスキーマに足す（`domain/model/network.ts`。0 以上の整数）
- [ ] `route.json` の版数を 2 に上げ、**付録 B の確定値**を入れる（`data/route.json`）
- [ ] 版数 1 を読んだら距離を未設定として扱う（`domain/network/load.ts`）
- [ ] R-13（すべての区間が距離を持つ）を足す。**版数 2 以上にのみ適用**（`domain/network/validate.ts`）
- [ ] 便の距離・走行距離・営業距離を導出する純関数を書く（[§4.4](#44-距離と輸送力の導出t-63t-64)）
- [ ] 設定ダイアログの「区間所要時間タブ」を「区間タブ」に改め、距離の欄を足す
  - [ ] **距離を変えても便の時刻は動かない**（「何便に効くか」は所要時間のときだけ出す）
- [ ] 運用の一覧に走行距離と営業距離を出す（`features/sidebar/BlockList.tsx`）

**受入条件**

- [ ] 付録 B の 6 つの行程が、区間の合計と一致する（テストで固定）
- [ ] 版数 1 の `route.json` が読める。距離の計算結果は `—`
- [ ] 距離が未設定の区間があると、合計を数で出さない
- [ ] どの区間が未設定かが区間タブに出る
- [ ] 距離の適用で時刻が変わらない
- [ ] ドメイン層のカバレッジ 100% を保つ
...
**目的**: 仕様書 v1.1 §6.2 を実装する（#162）。

**背景**: 定員を決めているのは**その便がどの系統を走るか**である。運用番号はその文書の中だけの名前であり、`A` の定員という概念が文書をまたいで意味を持たない。**`route.json` は触らない**——プロジェクトに置けば、Web 版でも変えられる。

**作業内容**

- [ ] `Project.patternCapacities: Record<string, number>` を足す（トップレベル、既定 `{}`）
- [ ] 既定値の定数を置く（**72。暫定であることをコメントに残す**）
- [ ] 延べ輸送力・断面輸送力を導出する純関数を書く（[§4.4](#44-距離と輸送力の導出t-63t-64)）
  - [ ] **回送便は数えない**
  - [ ] **運用番号が空欄の便は数えない。** 数えなかった件数を返す
  - [ ] 断面は区間ごと。時間範囲に入るかは**その区間を通る時刻**で判定する
- [ ] 系統ごとの定員を編集する画面を置く（営業パターンのみ。回送は並べない）

**受入条件**

- [ ] 定員を変えると輸送力が変わり、**`route.json` は書き換わらない**
- [ ] Web 版でも定員を変えられ、保存したファイルを開き直すと残っている
- [ ] `formatVersion` を上げずに、v1.0 が保存したファイルが読める
- [ ] 数えなかった便の件数が画面に出る
- [ ] 回送パターンが定員の一覧に出ない
- [ ] ドメイン層のカバレッジ 100% を保つ
...
**目的**: 仕様書 v1.1 §6.4・§6.5 を実装する（#166）。

**背景**: **フォーカスは視野ではない。** 視野に連動させると、拡大しただけで数が変わる。画面を送るたびに数が動く表は読めない。

**作業内容**

- [ ] `ViewSettings.focus: { enabled, from, to }` を足す（既定 無効・7:00〜22:00）
- [ ] 距離と輸送力の導出関数がフォーカス範囲を**引数で**受ける（[§4.4](#44-距離と輸送力の導出t-63t-64)）
  - [ ] 距離は**区間単位で切る**（区間の途中で按分しない）
  - [ ] 定員は**切らない**
- [ ] ダイヤグラムで範囲の外を薄く落とす（**隠さない**）
- [ ] サイドパネルに計算の節を置く
  - [ ] フォーカス範囲の指定をここに置く
  - [ ] 断面輸送力の表（区間 × 方向）
  - [ ] 数えなかった便の件数
...
- [ ] 拡大・縮小・スクロールで数が変わらない
- [ ] 範囲を有効にすると、範囲の外のスジが薄くなる（消えない）
- [ ] 範囲の外の便を選べる・動かせる・消せる
- [ ] **検証の指摘が範囲で絞られない**
- [ ] `22:00〜25:00` のような範囲を指定できる
- [ ] 範囲を変えても履歴に載らず、未保存にもならない
- [ ] 範囲がプロジェクトに保存され、開き直すと再現する
- [ ] 人数に小数が出ない

**規模**: L ／ **優先度**: v1 ／ **依存**: T-63, T-64
...
| N4 | 定員 72 が暫定のまま実測値と混同される | 誤った計算結果 | コメントに暫定であることを残す（T-64）。実測が出たら定数を差し替えるだけで済む形にする |
