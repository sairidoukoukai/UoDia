...
- [x] `Project.patternCapacities: Record<string, number>` を足す（トップレベル、既定 `{}`）
- [x] 既定値の定数を置く（**72。暫定であることをコメントに残す**）
- [x] 延べ輸送力・断面輸送力を導出する純関数を書く（`domain/block/capacity.ts`）
  - [x] **回送便は数えない**
  - [x] **時刻が未入力の便は数えない。** 数えなかった件数を返す
  - [x] 断面は区間ごと。時間範囲に入るかは**その区間を通る時刻**で判定する
- [x] 系統ごとの定員を編集する画面を置く（サイドパネルのパターン一覧。回送は並ばない）

**受入条件**

- [x] 定員を変えると輸送力が変わり、**`route.json` は書き換わらない**
- [x] Web 版でも定員を変えられ、保存したファイルを開き直すと残っている（`.uodia` に入る）
- [x] `formatVersion` を上げずに、v1.0 が保存したファイルが読める
- [ ] 数えなかった便の件数が画面に出る（**T-65 の計算パネルで出す**）
- [x] 回送パターンが定員の一覧に出ない
- [x] ドメイン層のカバレッジ 100% を保つ
...
- [x] `ViewSettings.focus: { enabled, from, to }` を足す（既定 無効・7:00〜22:00）
- [x] 距離と輸送力の導出関数がフォーカス範囲を**引数で**受ける（`domain/block/focus.ts`）
  - [x] 距離は**区間単位で切る**（区間の途中で按分しない）
  - [x] 定員は**切らない**
- [x] ダイヤグラムで範囲の外を薄く落とす（**隠さない**）
- [x] サイドパネルに計算の節を置く（`features/sidebar/CalcPanel.tsx`）
  - [x] フォーカス範囲の指定をここに置く
  - [x] 断面輸送力の表（区間ごと）
  - [x] 数えなかった便の件数
...
- [x] 拡大・縮小・スクロールで数が変わらない（`focus` は視野を見ない）
- [x] 範囲を有効にすると、範囲の外のスジが薄くなる（消えない）
- [x] 範囲の外の便を選べる・動かせる・消せる（当たり判定に手を入れていない）
- [x] **検証の指摘が範囲で絞られない**（`useValidationIssues` は `focus` を見ない）
- [x] `22:00〜25:00` のような範囲を扱える（導出は 24 時超を扱う）
- [x] 範囲を変えても履歴に載らず、未保存にもならない
- [x] 範囲がプロジェクトに保存され、開き直すと再現する
- [x] 人数に小数が出ない（定員を按分しない）
