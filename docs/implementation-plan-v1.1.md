...
**v1.0 の実装計画書を置き換えない。** タスク番号は T-55 の続き（**T-56〜T-68**。うち T-64・T-65 は見送り）とする。方針（[§2 開発方針](implementation-plan.md)）・技術的な設計判断（§3）・issue 化の方針（§7）は v1.0 の本書をそのまま引く。
