...
| 1.1 | 2026-08-13 | **後方互換性を切ってよいと言われたが、切らない**（[§3.2](#32-移行で埋めるのはその環境の-routejson)）。移行に要るネットワーク定義は **`loadProject` に既に渡っている**（`src/domain/io/load.ts:72`）ため、**切って浮く手間がほとんど無い。** 切って得られるものが小さく、失うもの（自分の書きかけが開けなくなる）が確実であるなら、切らないほうがよい。**申し出のうち、実際に手間の減るところだけ受ける**——上書きが消えることの通知（T-90）をやめ、移行のテストを 4 → 5 に絞る。 |
| 1.2 | 2026-08-13 | **T-89〜T-92 を実装した。** §3.1 の見立ては当たり、路線を読む側（描画・時刻の導出・検証・GTFS）は 1 行も変わっていない。実装して分かったことを 2 つ足す——**起動の順を変えた**（種を載せた時点で `ready` にすると、路線を持たない画面が一瞬出る。文書が揃ってから開ける）ことと、**`serializeNetworkDef` を落とした**（`route.json` を書く人が居なくなった）こと。 |
...
#### 切ってよいと言われたが、切らない（2026-08-13）

後方互換性は切ってよいと言われた。**切らない。**

```ts
// src/domain/io/load.ts:72 — いまの形
export function loadProject(json: string, network: NetworkIndex, …): LoadProjectResult
```

**移行に要るものは、既に渡っている。** 参照整合性の検査のために `NetworkIndex` を受け取っており、**4 → 5 の変換はそれをそのまま使える。** 変換の一覧を「ネットワークを束ねて作る」形にするだけで済む（変換関数は `unknown` しか受け取らないため、束ねるのは `MIGRATIONS` の側である）。

> **切って浮く手間がほとんど無い。** 得られるものが小さく、失うもの（自分の書きかけが開けなくなる）が確実であるなら、切らないほうがよい。

**申し出のうち、実際に手間の減るところだけ受ける。**

| | |
| --- | --- |
| 4 → 5 の変換 | **書く**（ほぼ無料であるため） |
| 版数 1〜3 の変換 | **触らない**（既にあり、通っている。消すことも手間である） |
| 上書きが消えることの通知（T-90） | **やめる**（[§3.3](#33-表示の上書きは移さない2026-08-11-決定)） |
| 移行のテスト | **4 → 5 に絞る**（1 〜 5 の全経路は追わない） |
...
**消えたことは伝えない**（2026-08-13 改め）。当初は一度だけ知らせることにしていたが、**後方互換性を切ってよいという判断**（[§3.2](#切ってよいと言われたが切らない2026-08-13)）のもとでは、そのための画面を作る理由が弱い。**選び直せば済む**。
...
- [ ] 変換の一覧を**ネットワーク定義から作る**形にする（`loadProject` は既に受け取っている）
...
| リスク | 対策 |
| --- | --- |
| **版数 5 の移行を誤ると、既存の文書が開けなくなる** | 版数 4 のファイルを**そのまま読むテスト**を置き、移行後の時刻が移行前と一致することを値で突き合わせる。**1 〜 3 は追わない**（[§3.2](#切ってよいと言われたが切らない2026-08-13)） |
