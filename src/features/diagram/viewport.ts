 ...
 * **動かしたいのは停留所線の位置だけである。**
 *
 * **幅はそこに何が積まれるかで決まる**（#167）。切れるもののうち最も大きい
 * はみ出しはつまみの半分（3.5px）だが、折返しの接続線が**外側へ段を成して
 * 積まれる**ため、その段数ぶんが要る。段は 7px ずつであり、24px は 3 段ぶんに
 * 当たる。
 *
 * **3 段で足りるという想定である。** 同じ停留所に 4 台以上が同時に留まると
 * 一番外の線は切れる。切れたことは絵から読めるため、起きたときに広げればよい
 * ——**起きるかどうか分からない段数ぶんを、いま画面から削る理由が無い。**
 */
export const AXIS_EDGE_MARGIN = 24;
