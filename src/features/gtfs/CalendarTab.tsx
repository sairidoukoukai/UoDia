 ...
 * ## 打った時点で履歴に載せる
 *
 * 適用ボタンを挟まない。触るのはプロジェクト（`Service.calendar`）であり、戻す
 * 手立てが <kbd>Ctrl</kbd>+<kbd>Z</kbd> にあるのだから、**打った時点で履歴に
 * 載せてよい**（§4.5.1）。
 *
 * 設定ダイアログが適用ボタンを挟むのは、あちらが `route.json` を書き換えるため
 * である。**この画面に `route.json` を触るタブはもう無い**（T-85、#221）。
 *
 * ## 積むのは 1 回だけ
 *
