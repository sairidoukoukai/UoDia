 ...
 *
 * **入れ物ではなく中身を測る。** 書庫は圧縮の効き方が OS ごとに違い、同じ数字
 * として比べられない。目標が言っているのは「配ったものが 30MB に収まるか」で
 * あり、それは**実行ファイル**の大きさで判断するのが素直である。
 *
 * **インストーラーはもう作らない**（T-95、#245）。残るのは実行ファイルと、
 * macOS の `.app` だけである。
 *
